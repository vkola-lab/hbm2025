...
class Mlp(nn.Module):
    def __init__(
        ...
        self.mlp_dim = mlp_dim or hidden_size
        self.hidden_size = hidden_size
        self.linear1 = nn.Linear(hidden_size, self.mlp_dim)
        self.linear2 = nn.Linear(self.mlp_dim, hidden_size)
        self.fn = nn.GELU()
...
class SwinTransformer(nn.Module):
    def __init__(
        ...
        mf_token1 = torch.zeros(
            [1, img_size*factor//2, img_size*factor//2, img_size*factor//2, 1], dtype=torch.float, requires_grad=True
        )
        self.mf_token1 = torch.nn.Parameter(mf_token1)

        mf_token2 = torch.zeros(
            [1, img_size*factor//4, img_size*factor//4, img_size*factor//4, 1], dtype=torch.float, requires_grad=True
        )
        self.mf_token2 = torch.nn.Parameter(mf_token2)

        mf_token3 = torch.zeros(
            [1, img_size*factor//8, img_size*factor//8, img_size*factor//8, 1], dtype=torch.float, requires_grad=True
        )
...
class Model(nn.Module):
    def __init__(
        ...
        self.num_modals = len(self.modal)
        self.num_cls = cfg.num_cls
        feature_size = cfg.transformer_dims
        self.img_size = (cfg.img_size,) * spatial_dims
        self.patch_size = cfg.patch_size
        _patch_size = (2,) * spatial_dims
        window_size = (7,) * spatial_dims

        import numpy as np

        for m, p in zip(self.img_size, _patch_size):
            for i in range(5):
        ...
        self.swinEncoder = SwinTransformer(
            ...
            attn_drop_rate=cfg.dropout_rate,
            drop_path_rate=cfg.dropout_rate,
            norm_layer=nn.LayerNorm,
            use_checkpoint=use_checkpoint,
            spatial_dims=spatial_dims,
            img_size=cfg.img_size
        )
        self.transformer_basic_dims = feature_size * 2 ** len(depths)
        embed_dims = [1, self.num_modals, 16]
        num_heads1 = [1, 2, 4, 8]
        qkv_bias = False
        qk_scale = None
        drop_rate = 0.0
        ...
        depths = [1, 1, 1]
        sr_ratios = [8, 4, 2, 1]
        mlp_img_size = 224 #self.img_size[0]
        ...
        dummy_input = torch.randn(1, self.num_modals, 1, *self.img_size)
        mask = torch.ones(1, self.num_modals, dtype=torch.long)
        _, _, _, _, _, mod_intra_x5, average_x5, x5_inter = self.forward(dummy_input, mask, features_only=True)
    ...
    def forward(self, x, mask, features_only=False, is_training=False):
        ...
        for idx, mod in enumerate(self.modal):
            mod_x1[mod], mod_x2[mod], mod_x3[mod], mod_x4[mod], mod_x5[mod] = self.swinEncoder(
                x[:, idx, :, :, :], self.normalize, m_label=mod
            )
            ic(mod_x1[mod].size(), mod_x2[mod].size(), mod_x3[mod].size(), mod_x4[mod].size(), mod_x5[mod].size())
            mod_token_x5[mod] = (
                mod_x5[mod].permute(0, 2, 3, 4, 1)
                .contiguous()
                .view(x.size(0), -1, self.transformer_basic_dims)
            )
        
            mod_token_x5[mod] = mod_token_x5[mod].unsqueeze(1).contiguous()
            ic(mod_token_x5[mod].size())

            out, H, W = self.patch_embed3(mod_token_x5[mod])
            ic(out.size(), H, W)
            for i, blk in enumerate(self.block3):
                out = blk(out, H, W)
            mod_intra_token_x5[mod] = self.norm3(out)
            mod_intra_token_x5[mod] = mod_intra_token_x5[mod].reshape(
                B, -1, H, W
            ).contiguous()

        
            mod_intra_x5[mod] = (
                mod_intra_token_x5[mod].view(
                    x.size(0), 1, self.transformer_basic_dims, self.num_modals, -1, 
                )
                # .permute(0, 4, 1, 2, 3)
                .contiguous()
            )
        ...
        x1 = self.masker(
            torch.stack(list(mod_x1.values()), dim=1), mask
        )  # BxMxCxHxWxZ
        ...
        x5_intra = self.masker(
            torch.stack(
                list(mod_intra_x5.values()), dim=1
            ),
            mask,
        )
        ic(torch.stack(list(mod_intra_token_x5.values()), dim=1).size())
        # x5_intra = self.masker(
        #     torch.stack(
        #         list(mod_intra_token_x5.values()), dim=1
        #     ),
        #     mask,
        # )
        ...
        mod_intra_x5 = torch.chunk(
            x5_intra, self.num_modals, dim=1
        )
        average_x5 = torch.sum(torch.stack(mod_intra_x5), dim=0)
        average_x5 = torch.div(average_x5, self.num_modals)
        ic(average_x5.size())

        # ic([inp.permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), -1 , self.transformer_basic_dims).size() for inp in mod_intra_x5])
        # multimodal_token_x5 = torch.cat([inp.permute(0, 2, 3, 4, 1).contiguous().view(x.size(0), 1, -1 , self.transformer_basic_dims) for inp in mod_intra_x5], dim=1)
        multimodal_token_x5 = torch.cat([inp.view(x.size(0), 1, -1 , self.transformer_basic_dims).contiguous() for inp in mod_intra_x5], dim=1)
        ic(multimodal_token_x5.size())
        # multimodal_token_x5 = multimodal_token_x5.view(x.size(0), self.patch_embed4.in_chans, -1, self.transformer_basic_dims)
        # ic(multimodal_token_x5.size())
        ...
        mod_intra_x5 = [mod_intra.permute(0,2,3,4,1).contiguous().view(B, -1) for mod_intra in mod_intra_x5]
        average_x5 = average_x5.permute(0, 2, 3, 4, 1).contiguous().view(B, -1)
        
        if features_only:
            return x1, x2, x3, x4, x5, mod_intra_x5, average_x5, x5_inter
